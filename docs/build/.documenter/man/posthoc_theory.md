...
# Post-Hoc Analysis Guide {#Post-Hoc-Analysis-Guide}

Post-hoc tests are performed after a significant omnibus test (like ANOVA or Chi-Square) to determine _which_ specific groups or categories differ. 

## 1. Parametric Post-Hoc Tests {#1.-Parametric-Post-Hoc-Tests}
...
| **:scheffe**    | Equal               | High         | Designed for _any_ linear contrast, not just pairwise. Very conservative for simple pairwise tests.                                               |
...
PostHocPar(groups; method=:tamhane)
```


## 2. Non-Parametric Post-Hoc Tests {#2.-Non-Parametric-Post-Hoc-Tests}
...
## 3. Contingency Table Post-Hoc {#3.-Contingency-Table-Post-Hoc}
...
### A. Row-wise Comparisons (`PostHocContingencyRow`) {#A.-Row-wise-Comparisons-PostHocContingencyRow}
...
### B. Cell-wise Residuals (`PostHocContingencyCell`) {#B.-Cell-wise-Residuals-PostHocContingencyCell}
...
## Compact Letter Display (CLD) {#Compact-Letter-Display-CLD}
